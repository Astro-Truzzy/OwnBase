  ...
  if (loading) {
    return (
      <section className="rounded-xl border border-border bg-surface/50 p-6 sm:p-8">
        <h2 className="text-lg font-medium text-foreground">Repository structure</h2>
        <div className="mt-4 flex flex-wrap gap-3">
          {[1, 2, 3, 4].map((i) => (
            <div
              key={i}
              className="h-14 w-28 animate-pulse rounded-lg bg-border"
            />
          ))}
        </div>
      </section>
  ...
  return (
    <section className="rounded-xl border border-border bg-surface/50 p-6 sm:p-8">
      ...
      <div className="mt-6 flex flex-wrap gap-4">
        <div className="rounded-xl border border-border bg-background/80 px-5 py-3">
          <span className="text-2xl font-semibold tabular-nums text-foreground">
            {totalFolders}
          </span>
          <span className="ml-2 text-sm text-muted">folders total</span>
        </div>
          ...
          return (
            ...
            >
              ...
              <div>
                ...
                <span className="ml-1.5 text-sm text-muted">{category}</span>
              </div>
            </div>
          );
        })}
