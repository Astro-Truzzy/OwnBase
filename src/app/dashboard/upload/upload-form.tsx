...
export function UploadForm() {
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    startTransition(async () => {
      ...
      if (result.success) {
        setMessage({ type: "success", text: "Project uploaded. It’s now stored in your environment." });
        form.reset();
      } else {
  ...
  return (
    <section className="rounded-xl border border-border bg-surface p-6 sm:p-8">
      ...
      <form onSubmit={handleSubmit} className="mt-6 space-y-4">
        <div>
          <label htmlFor="upload-name" className="block text-sm font-medium text-foreground mb-1">
            Project name
          </label>
          <input
            id="upload-name"
        ...
        <div>
          <label htmlFor="upload-file" className="block text-sm font-medium text-foreground mb-1">
            Zip file
          </label>
          <input
            id="upload-file"
            ...
            className="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm text-foreground file:mr-3 file:rounded-md file:border-0 file:bg-accent/10 file:px-3 file:py-1.5 file:text-sm file:font-medium file:text-accent focus:outline-none focus:ring-2 focus:ring-accent focus:ring-offset-2 focus:ring-offset-background"
            disabled={isPending}
          />
        </div>
        {message && (
